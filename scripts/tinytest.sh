...
export NLTK_DATA=/home/share/models/nltk_data
...
for dataset in "${datasets[@]}"; do
    ...
    SAVE_DIR="saved_models/PRoTeD/${dataset}_run"


    if [[ ! -f "$DATA_PATH" ]]; then
        echo "⚠️  Skip ${dataset}: train file not found: ${DATA_PATH}"
        continue
    fi
    if [[ ! -f "$EVAL_PATH" ]]; then
        echo "⚠️  Skip ${dataset}: test file not found: ${EVAL_PATH}"
        continue
    fi
    ...
    log_file="${LOG_DIR}/${dataset}_${ts}.log"
